...
class SettingsService implements FederalStateSettingsService, LockTimeEntriesSettingsService, SubtractBreakFromTimeEntrySettingsService, TimeEntrySettingsService {
    ...
    /**
     * Updates {@link TimeEntrySettings}.
     *
     ...
     * @return the updated {@link TimeEntrySettings}
     */
    TimeEntrySettings updateTimeEntrySettings(boolean commentEnabled, boolean breakIntegrated, int defaultBreakMinutes) {
        ...
        entity.setBreakIntegrated(breakIntegrated);
        entity.setDefaultBreakMinutes(defaultBreakMinutes);

        final TimeEntrySettingsEntity saved = timeEntrySettingsRepository.save(entity);
    ...
    private Optional<TimeEntrySettingsEntity> getTimeEntrySettingsEntity() {
        // `findFirst` is sufficient as there exists only one TimeEntrySettingsEntity per tenant.
        // however, the tenantId is handled transparently in the background. and we only have the public API of `findAll`.
    ...
    private static TimeEntrySettings toTimeEntrySettings(TimeEntrySettingsEntity entity) {
        return new TimeEntrySettings(
            entity.isCommentEnabled(),
            entity.isBreakIntegrated(),
