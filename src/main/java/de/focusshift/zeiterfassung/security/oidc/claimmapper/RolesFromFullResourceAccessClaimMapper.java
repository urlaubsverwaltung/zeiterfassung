...
class RolesFromFullResourceAccessClaimMapper implements RolesFromClaimMapper {
    ...
    private static final SimpleGrantedAuthority NEEDED_RESOURCE_ACCESS_ROLE = new SimpleGrantedAuthority(ZEITERFASSUNG_USER.name().toUpperCase());
    ...
    public List<GrantedAuthority> mapClaimToRoles(Map<String, Object> claims) {
        ...
        if (properties.isAuthorityCheckEnabled() && grantedAuthorities.stream().noneMatch(NEEDED_RESOURCE_ACCESS_ROLE::equals)) {
            final String prefixedNeededRole = NEEDED_RESOURCE_ACCESS_ROLE.toString().toLowerCase();
            LOG.error("User with sub '{}' has not required permission '{}' in '{}' to access zeiterfassung!", claims.get(SUB), prefixedNeededRole, grantedAuthorities);
            throw new MissingClaimAuthorityException(format("User has not required permission '%s' in '%s' to access zeiterfassung!", prefixedNeededRole, grantedAuthorities));
        }
